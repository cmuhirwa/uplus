...
class Examres{
	...
	function classes($scode){
		...
		$reb_marks = $conn->query("SELECT COUNT(SUBSTR(code, 8, 3)) num, SUBSTR(code, 8, 3) as class FROM 6marks WHERE code LIKE "$code%" GROUP BY class");
		while ($data = $reb_marks->fetch_assoc()) {
			//checking if this is correct three letter combination
			if(!strrpos("16", "test".$data['class']))
				$classes[$data['class']] = $data['num'];
		}
		var_dump($classes);
